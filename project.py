...
class Process:
    ...
    def __init__(self, rand, lamb):
        ...
        self.time_added = 0
        self.original_arrival_time = 0
        self.total_cpu_burst_time = 0
        for burst in range(cpu_bursts*2-1):
            if burst % 2 == 0:
                val = math.ceil(rand.next_exp())
                self.bursts.append(val)
                self.total_cpu_burst_time += val
            else:
...
class CPU:
    ...
    def reset(self):
        ...
        for i in range(process_num):
            ...
            self.total_cpu_burst_time += self.processes[i].total_cpu_burst_time
    ...
    def decimal_ceil(self, n):
        """ Ceiling round and format a float n """
        n_decimals = 3
        n_ceil = math.ceil((n * 10 ** n_decimals) / 1) / (10 ** n_decimals)
        return "{0:1.3f}".format(n_ceil)
    ...
    def output_file(self, current_time, algorithm):
        ...
        avg_burst_time = self.total_cpu_burst_time / self.num_bursts
        total_context_switch_time = self.num_context_switches * self.switch_time * 2
        avg_wait_time = self.total_wait_time / self.num_bursts
        avg_turnaround_time = self.total_turnaround_time / self.num_bursts
        utilization = self.total_cpu_burst_time / current_time * 100
        f.write(f"-- average CPU burst time: {self.decimal_ceil(avg_burst_time)} ms\n")
        f.write(f"-- average wait time: {self.decimal_ceil(avg_wait_time)} ms\n")
        f.write(f"-- average turnaround time: {self.decimal_ceil(avg_turnaround_time)} ms\n")
        ...
        f.write(f"-- CPU utilization: {self.decimal_ceil(utilization)}%\n")
        f.close()
        ...
        while(len(self.events_queue) != 0):
            ...
            # Note neither type of switching uses process_num, switching in draws from the start of the queue
                ...
                if self.current_process == None:
                    ...
                    self.total_wait_time += current_time - self.processes[
                        self.current_process].time_added - self.switch_time
                ...
                self.total_turnaround_time += current_time - self.processes[process_num].time_added + self.switch_time
                if running:
    ...
    def run_sjf(self):
            ...
            if event_type == "d_arrival":
                ...
                if not switching_out:
                    self.total_turnaround_time += current_time - self.processes[
                        process_num].time_added + self.switch_time
                    running = self.processes[self.current_process].finish_burst()
                    if running:
                        bursts_left = self.processes[process_num].get_remaining_cpu_bursts()
                        self.print_event(current_time, process_num, "completed a CPU burst; {} burst{} to go".format(bursts_left, "" if bursts_left == 1 else "s"))
        ...
        self.print_event(current_time, None, "Simulator ended for {}".format(self.algorithm), True)
        self.output_file(current_time, "SJF")
        ...
        while(len(self.events_queue) != 0):
            ...
            if event_type == "d_arrival":
                ...
                if self.current_process != None and not switching_out and tau < self.processes[self.current_process].remaining_time_estimate(current_time):
                    ...
                    self.processes[old_process].time_added = current_time + self.switch_time
                    heappush(self.events_queue, (current_time+self.switch_time, "switch_out", self.current_process))
                    switching_out = True
                else:
                    self.print_event(current_time, process_num, "completed I/O; added to ready queue")
                ...
                last_resolved = current_time
        self.print_event(current_time, None, "Simulator ended for {}".format(self.algorithm), True)
        self.output_file(current_time, "SRT")
    ...
    def run_rr(self):
        ...
        while(len(self.events_queue) != 0):
            ...
            if event_type == "d_arrival":
                ...
                if process_num != -1:
                    self.processes[new_process].time_added = current_time
                    self.process_queue.append(process_num)
                switching_out = False
                self.lock = False
                self.current_process = None
                ...
                if self.current_process == None:
                    ...
                    cpu_burst_time = self.processes[self.current_process].start_burst(current_time)
                    cpu_original_burst_time = self.processes[self.current_process].total_burst_time()
                    self.total_wait_time += current_time - self.processes[self.current_process].time_added
                    if cpu_burst_time == cpu_original_burst_time:
                        self.print_event(current_time, self.current_process, "started using the CPU for {}ms burst".format(cpu_burst_time))
                        self.num_bursts += 1
                    else:
                    ...
                    if cpu_burst_time <= self.rr_time_slice:
                        ...
                        heappush(self.events_queue, (current_time+self.rr_time_slice, "ba_rr_time_expired", self.current_process))
                else:
                    print("Error: Attempting to switch in process when lock should be in place", file=sys.stderr)
                    sys.exit()
            elif event_type == "ba_rr_time_expired":
                if len(self.process_queue) > 0:
                    self.num_preemptions += 1
                    old_process = self.current_process
                    ...
                    heappush(self.events_queue, (current_time+self.switch_time, "ba_switch_out", self.current_process))
                    switching_out = True
                else:
                    cpu_burst_time = self.processes[self.current_process].remaining_time(current_time)
                    self.print_event(current_time, None, "Time slice expired; no preemption because ready queue is empty")
                    if cpu_burst_time <= self.rr_time_slice:
                        heappush(self.events_queue, (current_time+cpu_burst_time, "a_cpu_finish", self.current_process))
                    else:
                        heappush(self.events_queue, (current_time+self.rr_time_slice, "ba_rr_time_expired", self.current_process))
                ...
                if last_resolved != current_time:
                    ...
                    self.lock = True
                last_resolved = current_time
            elif event_type == "zz_resolve_switch_out_append":
                if (not self.lock) and len(self.process_queue) != 0:
                    new_process = self.process_queue.pop(0)
                    heappush(self.events_queue, (current_time+self.switch_time, "b_switch_in", new_process))
                    self.lock = True
